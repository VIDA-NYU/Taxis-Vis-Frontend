...
import { checkFileExists, validateConfigStructure } from './ConfigModal.utils';
...
export const handleSelectExisting = async (
  ...
  try {
    ...
    validateConfigStructure(loadedConfig);

    const dbExists = await checkFileExists(loadedConfig.dbPath);
    if (!dbExists) {
      throw new Error(`Missing DuckDB database file at ${loadedConfig.dbPath}`);
    }

    const neighborhoodGeoJson = loadedConfig.geoJsonLayers.find(
      (layer) => layer.name.toLowerCase() === 'neighborhood'
    );

    if (!neighborhoodGeoJson) {
      throw new Error(`Missing GeoJSON layer with name "neighborhood"`);
    }

    const geoJsonExists = await checkFileExists(neighborhoodGeoJson.url);
    if (!geoJsonExists) {
      throw new Error(`Missing GeoJSON file at ${neighborhoodGeoJson.url}`);
    }
