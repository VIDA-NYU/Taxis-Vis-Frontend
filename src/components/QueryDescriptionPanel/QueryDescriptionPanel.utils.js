...
export function getNeighborhoodNameKeys(config) {
  ...
  if (
    config &&
    config.neighborhoodKeys &&
    Array.isArray(config.neighborhoodKeys)
  ) {
...
export const findIntersectingNeighborhoods = (
  regionGeoJSON,
  geoJsonLayers,
  config
) => {
...
export const findNeighborhoodContainingPoint = (
  pointCoordinates,
  geoJsonLayers,
  config
) => {
  ...
  return containingFeature
    ? getNeighborhoodName(containingFeature, config)
