import {initializeDuckDb, getDuckDB, insertFile, runQuery} from "duckdb-wasm-kit";
...
export const initializeDatabase = async (
    ...
    try {
        ...
        if (configType === "existing") {
            ...
            await initializeDuckDb({config: {path: loadedConfig.dbPath}, debug: false});
            ...
            if (selectedConfig.config.csvLocationCols) {
                await addGeoJsonColumns(duckdbInstance, quotedTable, selectedConfig.config.csvLocationCols);
            }
...
const addGeoJsonColumns = async (duckdbInstance, tableName, csvLocationCols) => {
    const {pickupLon, pickupLat, dropoffLon, dropoffLat} = csvLocationCols;

    await runQuery(duckdbInstance, `ALTER TABLE ${tableName}
